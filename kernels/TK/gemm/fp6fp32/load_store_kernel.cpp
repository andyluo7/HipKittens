...
void micro_tk(const micro_globals g) {
    ...
    if (threadIdx.x == 0) {
        // printf("hi\n");
        printf("fl[0][0] packed_per_thread: %d\n", tile_fl.tiles[0][0].packed_per_thread);
        printf("fl packed_per_tile: %d\n", tile_fl.packed_per_tile);
        printf("fl width: %d, height: %d\n", tile_fl.width, tile_fl.height);
        printf("fl[0][0] registers_per_thread: %d\n", tile_fl.tiles[0][0].registers_per_thread);

        printf("bf16[0][0] packed_per_thread: %d\n", tile_bf16.tiles[0][0].packed_per_thread);
        printf("bf16 packed_per_tile: %d\n", tile_bf16.packed_per_tile);
        printf("bf16 width: %d, height: %d\n", tile_bf16.width, tile_bf16.height);
        printf("bf16[0][0] registers_per_thread: %d\n", tile_bf16.tiles[0][0].registers_per_thread);
        
        printf("fp6[0][0] packed_per_thread: %d\n", tile_fp6.tiles[0][0].packed_per_thread);
        printf("fp6 packed_per_tile: %d\n", tile_fp6.packed_per_tile);
        printf("fp6 width: %d, height: %d\n", tile_fp6.width, tile_fp6.height);
        printf("fp6[0][0] registers_per_thread: %d\n", tile_fp6.tiles[0][0].registers_per_thread);
    }

    // if (threadIdx.x == 0) {
    //     printf("Final tile_fl[0][0] first few elements: %.3f %.3f %.3f %.3f %.3f\n",
    //            tile_fl.tiles[0][0].data[0].x, tile_fl.tiles[0][0].data[0].y,
    //            tile_fl.tiles[0][0].data[1].x, tile_fl.tiles[0][0].data[1].y,
    //            tile_fl.tiles[0][0].data[2].x);
    // }
    // copy(tile_fp6, tile_fl);
